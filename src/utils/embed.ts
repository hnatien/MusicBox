...
import { COLORS, QUEUE_PAGE_SIZE, APP_EMOJIS, formatAppEmoji, getEmojiUrl } from './constants.js';
...
export function createNowPlayingEmbed(
  ...
  const embed = new EmbedBuilder()
    .setColor(0x2b2d31)
    .setAuthor({
      name: song.channelName.toUpperCase(),
      iconURL: getEmojiUrl(APP_EMOJIS.MUSIC_NOTES),
    })
    .setTitle(song.title)
    .setURL(song.url)
...
export function createMixStartedEmbed(
  title: string,
  firstSong: Song,
  totalCount: number,
): EmbedBuilder {
...
export function createQueueEmbed(
  ...
  if (totalSongsCount > 0) {
    embed.setFooter({
      text: `${totalSongsCount} ${totalSongsCount === 1 ? 'song' : 'songs'} in queue · Page ${page}/${totalPages}`,
    });
  }
