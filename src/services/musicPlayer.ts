...
async function _play(
  ...
  try {
    ...
    if (existingMessage || (reusedMessage && queue.nowPlayingMessage)) {
      ...
      const textChannel = client.channels.cache.get(queue.textChannelId) as
        | TextBasedChannel
        | undefined;
      if (textChannel && 'send' in textChannel) {
        const message = await textChannel
          .send({ embeds: result.embeds, components: result.components })
          .catch(function (): void {});
        if (message) {
...
export function startProgressUpdate(guildId: string): void {
  ...
  queue.progressInterval = setInterval(async () => {
    ...
    try {
      ...
      logger.warn(
        `Could not update progress for guild ${guildId}: ${error instanceof Error ? error.message : String(error)}`,
      );
      stopProgressUpdate(guildId);
    }
  }, 15_000);
