...
type YtdlModule = {
  ...
  default?: { constants?: { YOUTUBE_DL_PATH?: string } };
};
...
try {
  const ytdl = (await import('youtube-dl-exec')) as unknown as YtdlModule;
...
if (rawEnvCookie) {
  try {
    ...
    logger.warn(
      `Failed to write YouTube cookie file: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
async function searchByQueryWithYtdlp(query: string, requestedBy: string): Promise<Song[]> {
  ...
  const result = await new Promise<YtdlpSearchResult>((resolve, reject) => {
    const args = [
...
export async function getInfoByUrl(url: string, requestedBy: string): Promise<Song> {
  ...
  const result = await new Promise<Record<string, unknown>>((resolve, reject) => {
    ...
    proc.on('error', (err: Error) => {
      ...
      reject(
        new Error(
          `Failed to spawn yt-dlp (${ytdlpBinary}): ${err.message}. Ensure it is installed: pip install yt-dlp`,
        ),
      );
    });
  });
...
export async function getPlaylistInfo(
  ...
  const result = await new Promise<YtdlpPlaylistResult>((resolve, reject) => {
    const args = [
      url,
      '--dump-single-json',
      '--flat-playlist',
    ...
    const timeout = setTimeout(() => {
      if (!proc.killed) {
        proc.kill('SIGKILL');
        reject(
          new Error(`yt-dlp playlist metadata timed out after ${config.YT_PLAYLIST_TIMEOUT_MS}ms`),
        );
      }
  ...
  const songs: Song[] = entries
    ...
    .map((entry) => ({
      ...
      url: entry.url?.startsWith('http')
        ? entry.url
        : `https://www.youtube.com/watch?v=${entry.url || entry.id}`,
...
export async function getAudioStream(url: string): Promise<AudioStreamResult> {
  ...
  throw new Error(
    `Failed to get audio stream after ${STREAM_RETRY_ATTEMPTS} attempts: ${lastError}`,
  );
}
...
function createYtdlpStream(url: string): Readable {
  ...
  const ffmpegArgs = [
    ...
    'pipe:0',
    '-analyzeduration',
    String(FFMPEG_ANALYZE_DURATION),
    '-probesize',
  ...
  const ytdlpProc: ChildProcess = spawn(ytdlpBinary, ytdlpArgs, {
